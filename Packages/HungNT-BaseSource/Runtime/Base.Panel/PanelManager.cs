...
namespace Base
{
    public partial class PanelManager : MonoSingleton<PanelManager>
    {
        ...
        {
            await UniTask.WhenAll(Enumerable.Select(stackPanels, p => p.Init()).ToList());
            await UniTask.WhenAll(Enumerable.Select(stackPanels, p => p.PostInit()).ToList());
            stackPanels.ForEach(p => p.Show().Forget());
        }

        public async UniTask<T> Create<T>(string address) where T : UIPanel
        {
            var panel = (await Addressables.InstantiateAsync(address, transform).Task).GetComponent<T>();
            await panel.Init();
